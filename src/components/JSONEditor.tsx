...
const JSONEditor: React.FC = () => {
  ...
  useEffect(() => {
    const fetchJSON = async () => {
      ...
      try {
        ...
        setError(`ডেটা লোড সমস্যা: ${err.message}`);
        const file = JSON_FILES.find(f => f.id === selectedFile);
  ...
  const labelMap: Record<string, string> = {
    ...
    date: 'তারিখ', dateEn: 'তারিখ (ইংরেজি)', year: 'বছর', month: 'মাস', time: 'সময়',
    priority: 'প্রাধান্য', category: 'ক্যাটাগরি', question: 'প্রশ্ন', answer: 'উত্তর', keywords: 'কীওয়ার্ড',
    role: 'পদবী', phone: 'ফোন', email: 'ইমেইল', address: 'ঠিকানা', username: 'ইউজারনেম', password: 'পাসওয়ার্ড',
    imageUrl: 'ছবি লিংক', thumbnail: 'থাম্বনেইল', caption: 'ক্যাপশন', audioUrl: 'অডিও লিংক',
    url: 'লিংক', pdfUrl: 'PDF লিংক', streamUrl: 'স্ট্রিম লিংক', location: 'স্থান',
    artist: 'শিল্পী', duration: 'সময়কাল', channelName: 'চ্যানেল', fileName: 'ফাইল নাম', pujaName: 'পূজার নাম'
  };
  ...
  const renderFormField = (key: string, value: any) => {
    ...
    if (key === 'id') return (
      <div key={key} className="mb-4">
        ...
        <input type="text" value={String(formData[key] || '')} disabled className="w-full px-3 py-2 border rounded-lg bg-gray-100" />
      </div>
    );

    if (typeof value === 'string' && (value.length > 100 || ['details', 'answer', 'description', 'address'].includes(key))) return (
      <div key={key} className="mb-4">
        <label className="block text-sm font-semibold text-gray-700 mb-2">{label}</label>
        <textarea value={String(formData[key] || '')} onChange={(e) => handleFieldChange(key, e.target.value)} rows={5}
          className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 resize-y" />
      </div>
    );
    ...
    if (key === 'category' && selectedFile === 'galleryImages') return (
      <div key={key} className="mb-4">
        <label className="block text-sm font-semibold text-gray-700 mb-2">{label}</label>
    ...
    if (Array.isArray(value)) return (
      <div key={key} className="mb-4">
        <label className="block text-sm font-semibold text-gray-700 mb-2">{label} <span className="text-xs text-gray-500">(কমা দিয়ে)</span></label>
    ...
    if (typeof value === 'number') return (
      <div key={key} className="mb-4">
        <label className="block text-sm font-semibold text-gray-700 mb-2">{label}</label>
        <input type="number" value={formData[key] || ''} onChange={(e) => handleFieldChange(key, parseFloat(e.target.value) || 0)}
          className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" />
      </div>
    );
    ...
    if (typeof value === 'boolean') return (
      <div key={key} className="mb-4 flex items-center">
        <input type="checkbox" checked={Boolean(formData[key])} onChange={(e) => handleFieldChange(key, e.target.checked)}
    ...
    return (
      <div key={key} className="mb-4">
        <label className="block text-sm font-semibold text-gray-700 mb-2">{label}</label>
        <input type="text" value={String(formData[key] || '')} onChange={(e) => handleFieldChange(key, e.target.value)}
          className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" />
      </div>
    );
  };
  ...
  const btnClass = (active: boolean) => `px-3 py-2 rounded-lg text-sm font-medium transition ${active ? 'bg-orange-500 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-orange-50'}`;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
        ...
        <p className="text-sm text-blue-700">আইটেম যোগ/মুছুন, এডিট করুন এবং JSON কপি করে GitHub এ আপলোড করুন।</p>
      </div>

      {error && <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg"><p className="text-red-800">❌ {error}</p></div>}

      <div className="bg-white rounded-xl p-4 shadow-lg">
        <label className="block text-sm font-bold text-gray-700 mb-3">📁 ফাইল নির্বাচন:</label>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-3">
          {JSON_FILES.map(file => (
            <button key={file.id} onClick={() => setSelectedFile(file.id)} className={btnClass(selectedFile === file.id)}>{file.label}</button>
          ))}
        </div>
        {currentFile && <p className="text-xs text-gray-600">📂 <code className="bg-gray-800 text-green-400 px-2 py-1 rounded">{currentFile.path}</code></p>}
      </div>
      ...
      <div className="bg-white rounded-xl p-4 shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleAddItem} className="flex items-center gap-1 px-3 py-1.5 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600">
        ...
        <select value={selectedItemIndex} onChange={(e) => setSelectedItemIndex(Number(e.target.value))}
          ...
          {jsonData.map((item: any, i: number) => (
            <option key={i} value={i}>#{i + 1} - {item.title || item.name || item.question || item.channelName || item.fileName || `আইটেম ${i + 1}`}</option>
          ))}
        </select>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center justify-between mb-4 pb-3 border-b-2 border-orange-500">
            ...
            <button onClick={handleSaveItem} className="flex items-center gap-1 px-3 py-1.5 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600">
              <Save className="w-4 h-4" /> সংরক্ষণ
            </button>
          </div>
          <div className="max-h-96 lg:max-h-[550px] overflow-y-auto pr-2">
            {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-gray-800">
            ...
            <button onClick={handleCopyJSON} className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600">
              {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              {copied ? '✅ কপি হয়েছে!' : '📋 কপি'}
            </button>
          </div>
          <pre className="bg-gray-900 text-green-400 p-4 text-xs font-mono overflow-auto max-h-96 lg:max-h-[550px]">
            <code>{generatedJSON}</code>
          </pre>
          <div className="bg-yellow-50 border-t-2 border-yellow-400 p-3">
            <p className="text-xs text-yellow-800">⚠️ <code className="bg-yellow-200 px-1 rounded">{currentFile?.path}</code> এ পেস্ট করুন</p>
          </div>
        </div>
      </div>
