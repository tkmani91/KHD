...
export function ReligiousSongs() {
  ...
  const handlePlay = (song: typeof songs[0]) => {
    // যদি audioUrl থাকে তাহলে HTML Audio চালাও
    if (song.audioUrl) {
        ...
        <div className="mb-10">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {featured.map(song => (
              <div
                key={song.id}
                className={`rounded-3xl overflow-hidden bg-gradient-to-br ${song.color} text-white shadow-2xl p-6 card-hover`}
              >
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <button onClick={() => handleDownload(song)} className="flex-1 bg-white text-gray-800 font-bold py-2.5 rounded-xl font-hind text-sm hover:bg-yellow-100 transition-colors flex items-center justify-center gap-2">
                    ⬇️ ডাউনলোড
                  </button>
                </div>
                {playingId === song.id && (
                        ...
                        {[...Array(8)].map((_, i) => (
                          <div
                            ...
                            style={{
                              ...
                              animation: `shimmer ${0.5 + Math.random() * 0.5}s ease-in-out infinite`,
                              animationDelay: i * 0.1 + "s"
                            }}
                          ></div>
                        ))}
                      </div>
                      <span className="text-xs font-hind text-white/70">বাজছে...</span>
                    </div>
                    <div className="w-full bg-white/20 rounded-full h-1.5">
                      <div className="bg-yellow-300 h-1.5 rounded-full w-1/3"></div>
                    </div>
                    <p className="font-bengali text-xs text-white/60 mt-2 italic">"{song.lyrics}"</p>
                  </div>
                )}
              </div>
            ))}
            ...
            >
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => handleDownload(song)} className="w-10 h-10 bg-orange-500/20 hover:bg-orange-500/40 rounded-xl flex items-center justify-center text-orange-300 transition-colors" title="ডাউনলোড">
                    ⬇️
                  </button>
                </div>
              </div>

              {/* Expanded - Lyrics & Player */}
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        <div className="flex gap-1 items-end">
                          {[...Array(12)].map((_, i) => (
                            <div
                              key={i}
                              className="w-1 rounded-full bg-gradient-to-t from-orange-500 to-yellow-300"
                              style={{
                                height: Math.random() * 16 + 4 + "px",
                                animation: `shimmer ${0.4 + Math.random() * 0.6}s ease-in-out infinite`,
                                animationDelay: i * 0.08 + "s"
                              }}
                            ></div>
                          ))}
                        </div>
                        <span className="text-xs font-hind text-orange-300 shimmer">🎵 বাজছে...</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-hind text-xs text-gray-500">০:০০</span>
                        <div className="flex-1 bg-white/10 rounded-full h-2">
                          <div className="bg-gradient-to-r from-orange-500 to-yellow-400 h-2 rounded-full w-1/4"></div>
                        </div>
                        <span className="font-hind text-xs text-gray-500">{song.duration}</span>
                      </div>
                    </div>
